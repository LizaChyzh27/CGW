...
function draw() {
    ...
    // Масштабування навколо точки (0.5, 0.5) з коефіцієнтом 1.5
    setTextureScaling(scaleCenter.x, scaleCenter.y, scaleFactor);
...
// Ініціалізація значень для масштабування
let scaleCenter = { x: 0, y: 0 };
let scaleFactor = 1.1;

// Швидкість зміни центра масштабування
...
function handleKeyPress(event) {
    switch (event.key.toLowerCase()) {
        case 'w': // Вгору
            scaleCenter.y += scaleSpeed;
            break;
        case 's': // Вниз
            scaleCenter.y -= scaleSpeed;
            break;
        case 'a': // Ліворуч
            scaleCenter.x -= scaleSpeed;
            break;
        case 'd': // Праворуч
            scaleCenter.x += scaleSpeed;
            break;
    ...
    scaleMessage.textContent = `x: ${scaleCenter.x.toFixed(2)}, y: ${scaleCenter.y.toFixed(2)} `;
...
// Передача параметрів масштабування
function setTextureScaling(centerX, centerY, scale) {
    gl.uniform2f(shProgram.iScaleCenter, centerX, centerY);
    gl.uniform1f(shProgram.iScaleFactor, scale);
}

function updateScaling(){
    const scale = parseFloat(document.getElementById('vScaleFactor').value);
    scaleFactor = scale;
...
function initGL() {
    ...
    shProgram.iScaleCenter = gl.getUniformLocation(prog, 'scaleCenter');
    shProgram.iScaleFactor = gl.getUniformLocation(prog, 'scaleFactor');
